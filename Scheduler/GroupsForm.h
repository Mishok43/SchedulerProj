	...
	private:
		...
		System::Void buttonAddTag_Click(System::Object^  sender, System::EventArgs^  e) {
			...
			if (n > 0)
			{
				...
				{
					for (int i = 0; i < n; i++)
						MainData.GroupsFormList[this->dataGridView->SelectedRows[i]->Index]->addTag(MainData.messageBoxInput);
		...
		System::Void buttonRemoveTag_Click(System::Object^  sender, System::EventArgs^  e) {
			...
			if (n > 0)
			{
				...
				{
					for (int i = 0; i < n; i++)
						MainData.GroupsFormList[this->dataGridView->SelectedRows[i]->Index]->removeTag(MainData.messageBoxInput);
